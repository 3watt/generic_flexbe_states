...
from moveit_msgs.msg import MoveGroupAction, MoveGroupGoal, Constraints, JointConstraint, PositionConstraint, MoveItErrorCodes
...
class MoveitToSpecificState(EventState):
	'''
	...
	def __init__(self, move_group = 'arm', action_topic = '/move_group'):
		'''
		Constructor
		'''
		super(MoveitToSpecificState, self).__init__(outcomes=['reached', 'planning_failed', 'control_failed'])
	...
	def on_enter(self, userdata):
		...
		self._joint_values = [0.5, 0.5, 0.0, 0.0]
		...
		for i in range(len(self._joint_names)):
			goal_constraints.joint_constraints.append(JointConstraint(joint_name=self._joint_names[i], position=self._joint_values[i]))
